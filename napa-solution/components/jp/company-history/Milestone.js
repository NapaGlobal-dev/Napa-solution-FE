...
export default function Milestone({
  ...
  function animate(time, current) {
    ...
    let times;
    if (distance > 0) times = timeArr.slice(current, time);
    else times = timeArr.slice(time, current).reverse();
    ...
    const lineAnimate = {
      ...
      options: {
        // timing options
        duration:
          distance > 0 ? timeDistance : timeDistance - circleDiameter * rate,
        fill: "forwards",
      },
    };
    ...
    ring.current.animate(
      [{ border: "#FFFFFF solid 3px" }, { border: "#412490 solid 3px" }],
      {
    ...
    times.forEach((time, index) =>
      time.current.animate(
        timeAnimates[index].keyframes,
        timeAnimates[index].options
      )
    );
  }
  ...
  return (
    <div className="milestone" style={milestone}>
      <div style={circle1} onClick={() => history(1)} ref={time1}>
        1
      </div>
