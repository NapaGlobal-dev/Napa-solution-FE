...
export default function Banner({ data }) {
  const banner = getData(data, /EmbeddedServices_Banner_Img/)[0];
  const title = getData(data, /EmbeddedServices_Banner_Title/)[0];
  const subtitle = getData(data, /EmbeddedServices_Banner_SubTitle/)[0];
  const content = getData(data, /EmbeddedServices_Banner_Content/)[0];
  const subcontent = getData(data, /EmbeddedServices_Banner_SubContent/)[0];
  useEffect(() => {
